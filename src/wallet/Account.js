...
const AccountAugmentedContext = React.createContext({account: 0, changeAccount:()=>{}})
...
function AccountAugmented({ children }) {
  ...
  return (
    <AccountAugmentedContext.Provider value={{account: account, changeAccount:changeAccount}}>
      {children}
...
function AccountProvider({ children }) {
  return (
      <AccountAugmented>{children}</AccountAugmented>
  )
}
